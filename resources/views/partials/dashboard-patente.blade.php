{{-- resources/views/partials/dashboard-patente.blade.php --}}
...
@foreach ($dados as $arranchamento)
    <section class="mb-8">
        ...
        @if($agrupamento !== 'default')
            <h3>Patente: {{$departmentOptions[(int)$arranchamento->department]  ?? $rankOptions[(int)$arranchamento->rank] }}</h3>
        @endif
            ...
            <tbody>
                @foreach (explode(',', $arranchamento->user_details) as $user_detail)
                    ...
                    <tr>
                        ...
                        @if($agrupamento !== 'default')
                            <td class="border px-8 py-4">{{ $departmentOptions[(int)$additionalInfo] }}</td>
                        @endif
                    </tr>
                @endforeach
            </tbody>
